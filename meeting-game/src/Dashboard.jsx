...
function Dashboard({ player, gameState, onLogout, onScoreUpdate, onOpenSettings, onChangeUser }) {
  ...
  async function updateScore(delta) {
    if (!isAdmin) return; // Only admins can update scores
  ...
  async function toggleQuest(questId) {
    if (!isAdmin) return; // Only admins can toggle quests
  ...
  async function addQuest() {
    if (!newQuest.trim() || !isAdmin) return;
    setAddingQuest(true);
    try {
      const response = await fetch(`${API_BASE}/quest`, {
        method: 'PUT',
  ...
  async function deleteQuest(questId) {
    if (!isAdmin) return; // Only admins can delete quests
  ...
  async function startEditQuest(quest) {
    if (!isAdmin) return; // Only admins can edit quests
    setEditingQuestId(quest.id);
    setEditingQuestText(quest.text);
  }

  async function saveEditQuest(questId) {
    if (!editingQuestText.trim() || !isAdmin) return;
    try {
      const response = await fetch(`${API_BASE}/quest`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
